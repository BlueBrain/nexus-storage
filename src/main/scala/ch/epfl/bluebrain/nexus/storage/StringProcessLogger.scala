...
/**
  ...
  */
class StringProcessLogger(cmd: List[String], arg: Option[String]) extends ProcessLogger {
  ...
  private def format(s: String, arg: Option[String]): String = arg match {
    case Some(a) => s"$a $s"
    case None    => s
  }
...
object StringProcessLogger {
  ...
  def apply(cmd: List[String], arg: String): StringProcessLogger = new StringProcessLogger(cmd, Some(arg))

  def apply(cmd: List[String]): StringProcessLogger = new StringProcessLogger(cmd, None)
